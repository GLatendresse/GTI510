(function () {
  ...
  function PasswordController($scope, $stateParams, UsersService, $location, Authentication, PasswordValidator, Notification) {
    ...
    function onRequestPasswordResetSuccess(response) {
      ...
      vm.credentials = null;
      Notification.success({ message: response.message, title: '<i class="glyphicon glyphicon-ok"></i> Courriel de réinitialisation du mot de passe envoyé avec succès !' });
    }

    function onRequestPasswordResetError(response) {
      // Show user error message and clear form
      vm.credentials = null;
      Notification.error({ message: response.data.message, title: '<i class="glyphicon glyphicon-remove"></i> Échec d\'envoi du courriel de réinitialisation du mot de passe !', delay: 4000 });
    }
    ...
    function onResetPasswordSuccess(response) {
      ...
      Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> Mot de passe réinitialisé avec succès !' });
      // And redirect to the index page
      $location.path('/password/reset/success');
    }

    function onResetPasswordError(response) {
      Notification.error({ message: response.data.message, title: '<i class="glyphicon glyphicon-remove"></i> Échec de réinitialisation du mot de passe !', delay: 4000 });
    }
  }
}());
